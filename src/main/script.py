...
class BibFile(object):                      # класс, содержащий основные параметры записей bib-файла
    def __init__(self, name, fldr=None, mode="r", err="ignore"):  # создание пути и открытие соответствующего файла
        if fldr is None:
            self.path = name
        else:
            self.path = fldr + "\\" + name
    ...
    keys = set()                            # будем сохранять cite_key, чтобы не было повторных записей
...
class BibFileParameters:                    # класс, хранящий параметры шаблонной записи
    def __init__(self, file: BibFile):
        ...
        self.type_case = WordOperations.get_word_case(f_words[0])  # получаем регистр типа
        ...
        tag_word = ""                       # тэг (слово)
        self.space_after_tag = 0            # пробелы перед тегом
...
class BibFileOperations:                    # класс операций, связанных с bib-файлами (здесь - запись в bib-файл)
    @staticmethod
    def write_to_file(dest: BibFile, f_line, param: BibFileParameters, text, not_multiline):
        ...
        if param.space_after_tag > 1:
            position = param.equals_pos - param.space_before_tag - len(f_words[0])  # кол-во пробелов м/у тегом и "="
        ...
        dest.file.write(" " * param.space_before_tag + f_words[0] + " " * position + "=" +
                        ...
                        param.tag_bracket[1] * not_multiline)
...
template = BibFile(input())                 # шаблон по первой записи в bib (н/р "..\test\resources\Template1.bib")
...
folder = input()                            # папка (н/р "..\test\resources\BibTeXFilesToEdit")
path = os.path.abspath(os.path.join(folder, os.pardir) + "\\Edited")  # путь к папке с результатами работы
    ...
    if files.endswith('.bib'):              # для каждого .bib файла
        ...
        while True:
            ...
            if result_bib.multi_line_text == 1:
                ...
                result_bib.file.write(' ' * result_bib.spaces_for_multi + line)  # выводим следующую строку с отступом
                continue
            ...
            if line[0] == '}':
                ...
                if result_bib.invalid_type == 1:
                    ...
                    continue
                result_bib.file.write("\n")     # ввиду особенности write_to_file и правила о запятых
            ...
            if line[0] == '@' and result_bib.inside == 0:
                result_bib.inside = 1
                cite_key = line[line.find('{') + 1:-1]  # cite_key (ключ цитирования) - то, что после скобки и до ','
                cite_key = cite_key.replace('\\', '').replace('/', '').replace('\'', '')
                if cite_key.isnumeric() == 1:    # ключ цитирования не может быть просто числом
                    result_bib.invalid_type = 1
                    continue
                if cite_key in result_bib.keys:  # если ключ цитирования уже был, то такую запись не берем
                    result_bib.invalid_type = 1
                    continue
                else:
                    result_bib.keys.add(cite_key)  # иначе, добавим ключ цитирования
                words = LineOperations.get_words(line)
                words[0] = WordOperations.word_to_case(words[0], parameters.type_case)
                result_bib.file.write("@" + words[0] + "{" + cite_key)
                continue
            ...
            if line[0] != '@' and result_bib.inside == 1:
                ...
                if result_bib.closing_char != '{' and result_bib.closing_char != '"':  # это случай типа: year = 1984
                    j = 1
                    while line[equal_pos + j] == ' ':  # ищем первую позицию текста тега
                        j += 1
                    if line[len(line) - 1] == ',':
